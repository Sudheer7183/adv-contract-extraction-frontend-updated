...
export class PdfLoader extends Component<Props, State> {
  ...
  componentDidMount() {
    console.log("load pdf file..", this.props.url);
    this.load();
  }
  ...
  componentDidUpdate({ url }: Props) {
    console.log("pdfLoader----->", url)
    if (this.props.url !== url) {
      console.log("loaddd---->", url);
  ...
  render() {
    ...
    console.log("pdf load pdfDoc--->", this.state.pdfDocument);
    console.log("pdf load error---->", this.state.error);
    return (
      <>
