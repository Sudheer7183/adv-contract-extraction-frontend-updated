...
export default function DocPieChart() {
    ...
    useEffect(() => {
        request(`${BASEURL}graphql/`, doctype_pie, {}, {Authorization: `Bearer ${localStorage.getItem('Token')}`}).then((res: any) => setdata(res.doctypePiechart))
    }, [])
    ...
    for (var i = 0; i < lenn; i++) {
        const fs = data[i].contractType == null ? "Yet to be Processed" : data[i].contractType.contractTypeName
    ...
    return (
        <Plot
            ...
            onClick={HandleClick}
            layout={{
                width: 700, height: 500, title: 'Files by Contract Type',
                font: {
