...
function BarChartView() {
    ...
    useEffect(() => {
        request(`${BASEURL}graphql/`, project_bar, {}, {Authorization: `Bearer ${localStorage.getItem('Token')}`}).then((res: any) => setdata(res.projectBarchart))
    }, [])
    ...
    for (let a = 0; a < lenn; a++) {
        const pn = data[a].project.projectName
        project.push(pn)
    ...
    for (let o = 0; o < lenn; o++) {
        option.push({ x: project, y: newcount[o], name: doctype[o], type: "bar" });
    }
    ...
    for (let d = 0; d < doctype.length; d++) {
        ...
        option1.push({ x: project, y: req_y, name: req_doctype, type: "bar", hoverinfo: "x+y" });
    }
    ...
    console.log("project barchart pp--->", pp)
    ...
    return (
        <Plot
            ...
            onClick={HandleClick}
            layout={{
                width: 700, height: 300, title: 'Projects by Contract Type',
                font: {
