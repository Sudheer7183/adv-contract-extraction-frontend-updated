...
const UserEditModalForm: FC<Props> = ({ user, isUserLoading }) => {
  ...
  useEffect(() => {
    ...
    request(`${BASEURL}graphql/`, classificationFile, {}, {Authorization: `Bearer ${localStorage.getItem('Token')}`}).then((file: any) => setFile(file.allClassificationFiles))
  }, [])
  ...
  for (let j = 0; j < file.length; j++) {
    const cf = file[j]?.jsonFile
    const cf1 = file[j]?.jsonFile
    const id = file[j]?.id
    ...
    <>
      <form id='kt_modal_add_user_form' className='form' onSubmit={onSubmit} noValidate>
        ...
        >
          ...
          <div className='fv-row mb-7'>
            {/* begin::Label */}
            <label className='required fw-bold fs-6 mb-2'>Choose Classification File</label>
            {/* end::Label */}
