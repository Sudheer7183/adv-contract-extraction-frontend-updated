...
const RepoFileDelete =gql`
mutation RepoDeleteFile($id: Int!) {
  RepoDeleteFile(id: $id) {
    ok
  }
}
`
...
const DocumentListHead = () => {
    ...
    const Handledelete = async (file_id: any) => {
        request(`${BASEURL}graphql/`, RepoFileDelete, { id: file_id }, {Authorization: `Bearer ${localStorage.getItem('Token')}`})
            .then((response: any) => {
                console.log("delete file response", response);
                setOpen(true);
                console.log("delete file response", response);
                if (response?.RepoDeleteFile?.ok) {
