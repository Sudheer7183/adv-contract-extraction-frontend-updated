...
const UploadForm: FC<Props> = ({ id }) => {
    ...
    const projectName = data.filess?.edges[0].node.projectName
    ...
    const filenew = (name: any) => {
        ...
        let value = null
        if (data?.filess?.edges?.length == 0) {
            value = true
            return value
        }
        for (var j = 0; j < data?.filess?.edges?.length; j++) {
            console.log("data.files.edges[j].node.fileName", data.filess.edges[j].node.fileName);
            if (data.filess.edges[j].node.fileName !== name.replace(/(\.\w+)+$/, "")) {
