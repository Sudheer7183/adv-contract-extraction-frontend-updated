...
function calculateIsAllDataSelected<T>(data: Array<T> | undefined, selected: Array<ID>, selected1: Array<ID>): boolean {
  if (!data) {
    return false
  }

  return data.length > 0 && data.length === selected.length && data.length === selected1.length;
}
...
function groupingOnSelect(
  ...
  selected1: Array<ID>,
  setSelected1: Dispatch<SetStateAction<Array<ID>>>,
  ...
  if (selected1.includes(id)) {
    setSelected1(selected1.filter((itemId) => itemId !== id))
  } else {
    const updatedSelected = [...selected1]
    updatedSelected.push(id)
    setSelected1(updatedSelected)
  }
...
function groupingOnSelectAll<T>(
  ...
  setSelected1(data.filter((item) => item.id).map((item) => item.id))
}
